...
def value_tensors(state: dict) -> dict[str, torch.Tensor]:
    ...
    cards, monsters, interactions = (
        state["cards"],
        state["monsters"],
        state["card_monster_interactions"],
    )
    return {
        ...
        "card_selection_task": torch.tensor(
            state["card_selection_task"], dtype=torch.long
        ),
        ...
        "interaction_cards": torch.tensor(
            [x["card_index"] for x in interactions], dtype=torch.long
        ),
        "interaction_monsters": torch.tensor(
            [x["monster_index"] for x in interactions], dtype=torch.long
        ),
        "interaction_numeric": torch.tensor(
            [x["numeric"] for x in interactions], dtype=torch.float32
        ).reshape(-1, 6),
    }
