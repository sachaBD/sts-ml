...
int main(int argc, char** argv) {
    if (argc != 5) {
        std::cerr << "usage: play_entry_mcts_neural source.jsonl entry_id_or_deck_signature combat_seed simulations\n";
        return 2;
    }
    int skipped = 0;
    ...
    stsrl::MctsAgent agent{
        seed,
        {.simulations = simulations, .rollout_limit = 512},
        evaluate_leaf,
    };
    ...
    write_frame({
        {"type", "result"}, {"mode", "neural"}, {"projection", "deck_hp_only"}, {"entry_id", found->entry_id}, {"deck_signature", found->deck_signature}, {"source_seed", found->source_seed}, {"combat_seed", seed}, {"starting_hp", found->hp}, {"starting_max_hp", found->max_hp},
        {"won", environment.won()},
