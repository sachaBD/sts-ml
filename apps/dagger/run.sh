#!/usr/bin/env bash
# Launch a DAgger collection run (slop_docs/apps/dagger.md); inputs = [learner run, its source runs].
set -euo pipefail
...
mapfile -t inputs < <(.venv/bin/python apps/dagger/generate.py "$config" --inputs)
run_args=(--live)
...
exec .venv/bin/python -m sts_combat_rl.run "$schema" "$id" "${run_args[@]}" -- apps/dagger/job.sh "$config" --out '{out}'
