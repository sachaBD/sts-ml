...
// deterministic simulator, an upper-bound teacher, not a fair player.
...
// The search for `leaf` (validated); `net` must outlive the result.
...
// rollout_steps. teacher::settings (teacher_search.hpp) adds the recording settings.
nlohmann::json search_settings(const Leaf& leaf);
